...
{
	return m_handlers.contains(opcode) > 0;
}
...
void RegisterBuiltInOperations()
{
	...
	reg.Register(
		OpCode::OP_ADD,
		std::make_unique<BinaryArithmeticHandler>(
			OpCode::OP_ADD,
			"ADD",
	...
	reg.Register(
		...
		std::make_unique<BinaryArithmeticHandler>(
			OpCode::OP_DIVIDE,
			"DIV", [](const Value& l, const Value& r) {
				if (const auto rv = ValueHelper::As<double>(r); rv == 0.0)
				{
