...
{
	const uint16_t nameIdx = chunk.AddConstant(std::make_shared<const std::string>("x"));
	chunk.WriteConstant(42.0);
	chunk.Write(OP_DEFINE_GLOBAL);
	chunk.WriteOperand(OP_DEFINE_GLOBAL, nameIdx);

	chunk.Write(OP_GET_GLOBAL);
	chunk.WriteOperand(OP_GET_GLOBAL, nameIdx);
...
{
	const uint16_t nameIdx = chunk.AddConstant(std::make_shared<const std::string>("g"));
	...
	chunk.WriteOperand(OP_SET_GLOBAL, nameIdx);
...
{
	const uint16_t nameIdx = chunk.AddConstant(std::make_shared<const std::string>("not_exists"));
	chunk.Write(OP_GET_GLOBAL);
	chunk.WriteOperand(OP_GET_GLOBAL, nameIdx);
