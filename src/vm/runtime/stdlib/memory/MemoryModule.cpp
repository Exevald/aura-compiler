	...
	return std::visit(
		[&]<typename T0>(const T0& current) -> bool {
			...
			if constexpr (std::is_same_v<T, std::monostate> || std::is_same_v<T, bool>
				...
				|| std::is_same_v<T, TaskPtr>
				|| std::is_same_v<T, ChannelPtr>
				|| std::is_same_v<T, ContextPtr>
				|| std::is_same_v<T, ModulePtr>)
			{
				return true;
			}
			else if constexpr (
				std::is_same_v<T, PointerPtr> || std::is_same_v<T, IteratorPtr>
				|| std::is_same_v<T, ListenerPtr> || std::is_same_v<T, ConnectionPtr>
				|| std::is_same_v<T, MySqlConnectionPtr> || std::is_same_v<T, MySqlPoolPtr>
				|| std::is_same_v<T, MySqlStatementPtr> || std::is_same_v<T, MySqlResultPtr>
				|| std::is_same_v<T, MySqlRowPtr>
				|| std::is_same_v<T, RabbitMqConnectionPtr> || std::is_same_v<T, RabbitMqMessagePtr>)
			{
				return false;
			}
...
void MemoryModule::Install(SharedRuntime& runtime)
{
