...
ASTNodePtr ASTBuilder::SimplifyMapLit(RawNode& raw)
{
	auto map = std::make_unique<MapLiteralNode>();

	std::function<void(RawNode*)> flattenEntries = [&](RawNode* node) {
		if (!node || node->children.empty())
		{
			return;
		}

		if (node->ruleName == "map_entry" && node->children.size() >= 3)
		{
			map->entries.emplace_back(
				Simplify(std::move(node->children[0])),
				Simplify(std::move(node->children[2])));
			return;
		}

		for (auto& child : node->children)
		{
			if (auto* nestedRaw = dynamic_cast<RawNode*>(child.get()))
			{
				flattenEntries(nestedRaw);
			}
		}
	};
	...
	for (auto& child : raw.children)
	{
		...
		{
			if (map->keyTypeName.empty())
			{
				map->keyTypeName = ExtractType(childRaw);
			}
			else if (map->valueTypeName.empty())
			{
				map->valueTypeName = ExtractType(childRaw);
			}
			continue;
		}

		if (childRaw->ruleName == "map_entry_list_opt")
		{
			flattenEntries(childRaw);
		}
	}
...
{
	std::vector<TransactionNode::Region> regions;
	ASTNodePtr body;

	auto appendRegion = [&regions](bool usesShared, const std::string& name) {
		if (name.empty())
		{
			return;
		}
		if (!regions.empty()
			&& regions.back().usesShared == usesShared
			&& regions.back().name == name)
		{
			return;
		}
		regions.push_back(TransactionNode::Region{ usesShared, name });
	};
	std::function<void(const ASTNode*)> collectRegions = [&](const ASTNode* node) {
		const auto* rawNode = dynamic_cast<const RawNode*>(node);
		if (!rawNode)
		{
			return;
		}
		if (rawNode->ruleName == "region_expr")
		{
			...
			appendRegion(usesSharedRegion, regionName);
			return;
		}
		for (const auto& child : rawNode->children)
		{
			collectRegions(child.get());
		}
	};
	...
	for (auto& child : raw.children)
	{
		...
		{
			if (childRaw->ruleName == "region_expr" || childRaw->ruleName == "region_expr_list"
				|| childRaw->ruleName == "region_expr_list_tail")
			{
				collectRegions(childRaw);
			}
	...
	return std::make_unique<TransactionNode>(std::move(regions), std::move(body));
}
...
ASTNodePtr ASTBuilder::SimplifyUnary(RawNode& raw)
{
	...
	if (raw.children.size() == 2)
	{
		if (const auto* opLeaf = dynamic_cast<LeafNode*>(raw.children[0].get()))
		{
			if (opLeaf->value == "await")
			{
				return std::make_unique<AwaitExprNode>(
					Simplify(std::move(raw.children[1])));
			}
			if (opLeaf->value == "go")
			{
				return std::make_unique<GoExprNode>(
					Simplify(std::move(raw.children[1])));
			}
		}
