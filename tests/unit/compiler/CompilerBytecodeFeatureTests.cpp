...
TEST_F(CompilerTest, BytecodeIterLoopPopsIterationValueBeforeNextStep)
{
	const auto root = ParseCode(
		"fn walk(list: [int]) {"
		"  iter (item of list) {"
	...
	EXPECT_TRUE(ChunkContainsOpcode(chunk, OpCode::OP_ITER_NEXT));
	EXPECT_TRUE(ChunkContainsOpcode(chunk, OpCode::OP_POP));
}
